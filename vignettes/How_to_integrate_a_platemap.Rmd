...
```{r}
bruh<-NormaliseForIC50::generate_plate_map("../Validation/",mode = "return")
bruh

wells <- paste0("A", rep(3:12))
wells

plate_expanded_wells <- tidyr::crossing(
  Plate_Name = unique(bruh$Plate_Name),
  Well=wells
)

tidyr::complete(bruh,Plate_Name,Well=wells)

merge(bruh %>% dplyr::select(-Well),
      plate_expanded_wells) %>%
  dplyr::arrange(as.numeric(gsub("\\D","",Plate_Name)),
                 as.numeric(gsub("\\D","",Well)))

library(magrittr)
bruh <- dplyr::select(bruh, -one_of(names(plate_expanded_wells))) %>% dplyr::bind_cols(plate_expanded_wells)

bruh
```
you can also choose to write to an excel file.
```{r}
#NormaliseForIC50::generate_plate_map("../Validation/",mode="write",output_plateMap_file = "../Validation/validation_output/example_generated_platemap.xlsx")
